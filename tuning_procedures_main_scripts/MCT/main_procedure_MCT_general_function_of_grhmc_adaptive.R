    ...
    if(t > 0){ # when qbar_initial is equal to initialized median, we get a root right at the start. To avoid this, we set this condition here. 
      ...
      if(whichroot == 1){ # standard momentum refresh event
        ...
        if(t > proportion_time_until_adaptive_start * T){
          ...
          if(any(n_q_root_median >= 2)){ # only run this whenever one of the coordinates has crossed the median twice such that we have one registered time difference between two median crossing events. 
          ...
          } else { # if none of the coordinates have been registered with at least one value of time between crossing median, we just keep everything unchanged
