# First step - Tuning m and S using ISG

first_step_grhmc_transformed_function_ISG <- function(log_target_grad, 
                                                      lambda, 
                                                      n_parameters = NULL,
                                                      T = 5000, 
                                                      n_samples = 1000,
  ...
  # state[(4 + 6*n_dim):(4 + 7*n_dim - 1)]: \int gradient of original log target density ^2 dt
  # state[(4 + 7*n_dim):(4 + 8*n_dim - 1)]: m - mean vector where elements should reflect the mean of the original position coordinates
  # state[(4 + 8*n_dim):(4 + 9*n_dim - 1)]: S - diagonal matrix where the diagonal elements should reflect the marginal standard deviation of the original position coordinates
  tryCatch(
    {
  ...
  ode <- function(t, state, parms){
    ...
    eval_log_target_grad <- log_target_grad(state[(4 + 7*n_dim):(4 + 8*n_dim - 1)] + state[(4 + 8*n_dim):(4 + 9*n_dim - 1)] * state[4:(4 + n_dim - 1)])
    ...
    ret <- c(
      ...
      state[(4 + 7*n_dim):(4 + 8*n_dim - 1)] + state[(4 + 8*n_dim):(4 + 9*n_dim - 1)] * state[4:(4 + n_dim - 1)], # \int q dt = \int m + Sqbar dt
      (state[(4 + 7*n_dim):(4 + 8*n_dim - 1)] + state[(4 + 8*n_dim):(4 + 9*n_dim - 1)] * state[4:(4 + n_dim - 1)])^2, # \int q^2 dt = \int (m + Sqbar) dt
      eval_log_target_grad^2, # \int gradient of log target density of q original ^2 dt
  ...
  at.event <- function(t, state, parms){
    ...
    if(t > proportion_time_until_adaptive_start * T){
      ...
      proposed_s_adaptive <- sqrt(1 / (state[(4 + 6*n_dim):(4 + 7*n_dim - 1)] / t))
      ...
      newState <- c(
        ...
        state[(4 + 2*n_dim):(4 + 7*n_dim - 1)], # continue integrating all the quantities defined 
  ...
  y0 <- c(
    ...
    int_grad_original_log_target_squared = rep(0, n_dim),
    m = m_adaptive,
    s = s_adaptive
  )
