...
source("simulation_experiments_scripts/section_4_1/G4/normal_log_target_grad.R")

general_grhmc_normal_output <- general_grhmc_rstan_monitor_samples_stored(
  export_result = "simulation_experiments_scripts/section_4_1/G4/general_grhmc_normal_output.RDS",
  ...
  mct_step_gamma_elements_dual_averaging_first_run = rep(10, 10), 
  mct_step_gamma_elements_dual_averaging_second_run = rep(25, 10),
  mct_step_mu_log_scale_second_run = TRUE,
  mct_step_mu_scale_factor_second_run = 1.1,
  lambda_adaptive_step_sim_time = 5000
)

general_grhmc_normal_output <- readRDS("simulation_experiments_scripts/section_4_1/G4/general_grhmc_normal_output.RDS") # If the above has already been run, then read the generated object using this line

# RStan summary table where 50000 out of 100000 samples are considered from each chain

rstan_table <- rstan_summary_table(RDS_object = general_grhmc_normal_output, sample_grid = 2, name = "G4")
rstan_table

saveRDS(rstan_table$min_max_summary_table, file = "simulation_experiments_scripts/section_4_1/G4/relevant_rstan_table_G4.RDS")
