...
source("simulation_experiments_scripts/section_4_1/G3/normal_log_target_grad.R")

general_grhmc_normal_output <- general_grhmc_rstan_monitor_samples_stored(
  export_result = "simulation_experiments_scripts/section_4_1/G3/general_grhmc_normal_output.RDS",
...
general_grhmc_normal_output <- readRDS("simulation_experiments_scripts/section_4_1/G3/general_grhmc_normal_output.RDS") # If the above has already been run, then read the generated object using this line

# RStan summary table where 50000 out of 100000 samples are considered from each chain

rstan_table <- rstan_summary_table(RDS_object = general_grhmc_normal_output, sample_grid = 2, name = "G3")
rstan_table

saveRDS(rstan_table$all_summary_table, file = "simulation_experiments_scripts/section_4_1/G3/relevant_rstan_table_G3.RDS")
