...
source("simulation_experiments_scripts/section_4_3/F1/funnel_log_target_grad.R")

general_grhmc_funnel_output <- general_grhmc_rstan_monitor_samples_stored(
  export_result = "simulation_experiments_scripts/section_4_3/F1/general_grhmc_funnel_output.RDS",
...
general_grhmc_funnel_output <- readRDS("simulation_experiments_scripts/section_4_3/F1/general_grhmc_funnel_output.RDS") # If the above has already been run, then read the generated object using this line

# RStan summary table where 50000 out of 100000 samples are considered from each chain

rstan_table <- rstan_summary_table(RDS_object = general_grhmc_funnel_output, sample_grid = 2, name = "F1")
rstan_table

saveRDS(rstan_table$all_summary_table, file = "simulation_experiments_scripts/section_4_3/F1/relevant_rstan_table_F1.RDS")
